<?php
class Modelo_Hospital extends CI_Model {
	private $tabla;
	private $prefijo;
	
	function __construct(){
		$this->tabla = "hospitales";
		//$this->prefijo = substr($this->tabla, 0, 2) . "_";
		parent::__construct();
	}

	public function getLastId(){
		$this->db->select_max("id_hospital","maximo");
    ...
    public function eliminar($codigo){
    	$this->db->where('id_hospital', $codigo);
		$this->db->delete($this->tabla);
    }

    public function obtenerHospital($nombre_hospital){
    	$this->db->select('*');
    	$this->db->from($this->tabla);
    	$this->db->like('hos_nombre',$nombre_hospital);
    	$result = $this->db->get();
