<?php
class Modelo_Aporteinsumo extends CI_Model {
	private $tabla;
	private $prefijo;
	
	function __construct(){
		$this->tabla = "aporte_insumo";
		//$this->prefijo = substr($this->tabla, 0, 2) . "_";
		parent::__construct();
	}
	
	public function getLastId(){
		$this->db->select_max("id_aporteinsumo","maximo");
    ...
    public function eliminar($codigo){
    	$this->db->where('id_aporteinsumo', $codigo);
		$this->db->delete($this->tabla);
    }

    public function obtener_AporteInsumo($id_insumo){
    	$this->db->select('*');
