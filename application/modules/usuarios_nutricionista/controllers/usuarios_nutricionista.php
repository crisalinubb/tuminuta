...
class Usuarios_nutricionista extends CI_Controller {
	...
	public function index($pagina = 1){
		#Title
		$this->layout->title('Nutricionistas');

		#Metas
		$this->layout->setMeta('title','Nutricionistas');
		$this->layout->setMeta('description','Nutricionistas');
		$this->layout->setMeta('keywords','Nutricionistas');
		...
		$contenido['usuarios'] = $this->objUsuario->listar(array("id_perfil" =>3, "id_unidad" => $this->session->userdata("usuario")->id_unidad));

		$contenido['datos'] = $this->objUsuario->listar(array("id_perfil" =>3, "id_unidad" => $this->session->userdata("usuario")->id_unidad ), $pagina, $config['per_page']);
	...
	public function agregar($codigo = false){
		...
		if($this->input->post()){
			...
			#js
			$this->layout->js('js/sistema/usuarios_nutricionista/agregar.js');

			#nav
			$this->layout->nav(array("Nutricionistas "=> "usuarios_nutricionista", "Agregar Usuarios" =>"/"));
	...
	public function editar($codigo = false){
		...
		if($this->input->post()){
			...
			#js
			$this->layout->js('js/sistema/usuarios_nutricionista/editar.js');
			...
			#nav
			$this->layout->nav(array("Nutricionistas "=>"usuarios_nutricionista", "Editar Usuarios" =>"/"));
	...
	public function eliminar($codigo = false){
		if(!$codigo) redirect(base_url() . "usuarios_nutricionista/");
			...
			$contenido['pagination'] = $this->pagination->create_links();
			$contenido['usuarios'] = $this->objUsuario->listar(array("id_perfil" =>3 ));
	...
	public function busqueda(){
		...
		$contenido['datos'] = $this->objUsuario->obtenerUsuario_nutricionista($rut,$this->session->userdata("usuario")->id_unidad);
