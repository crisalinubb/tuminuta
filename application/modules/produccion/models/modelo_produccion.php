...
class Modelo_Produccion extends CI_Model {
	...
	public function obtener_insumos_desayuno($total, $receta){
		$result= $this->db->query('SELECT Insumos.nombre, (Insumos_por_Receta.cantidad * '.$total.') as Total, Insumos.id_unidad_medida FROM Insumos_por_Receta INNER JOIN Insumos ON Insumos_por_Receta.id_insumo = Insumos.id_insumo where Insumos_por_Receta.id_receta = '.$receta.'');
    	//die($this->db->last_query());
    	return $result->result();
	}
	...
	public function obtener_insumos_formula_lactea($total, $formula, $comp1, $comp2, $comp3){
		$result= $this->db->query('SELECT Insumos.nombre, (Insumos_por_Receta.cantidad * '.$total.') as Total, Insumos.id_unidad_medida FROM Insumos_por_Receta INNER JOIN Insumos ON Insumos_por_Receta.id_insumo = Insumos.id_insumo where Insumos_por_Receta.id_receta IN ('.$formula.','.$comp1.','.$comp2.','.$comp3.')');
    	//die($this->db->last_query());
    	return $result->result();
	}
