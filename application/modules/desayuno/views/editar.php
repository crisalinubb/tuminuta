...
<form id="form-editar" name="form-editar" method="post" class="form-horizontal">
  <fieldset>
    ...
    <div class="form-group">
      <div class="col-sm-10">
         <input type="hidden" id="codigo" name="codigo" class="form-control validate[required]" value="<?php echo $desayunos->id_desayuno; ?>" />
      </div>
    </div>
      ...
      <div class="col-sm-4">
        <select id="codigo_receta" name="codigo_receta" class="selectpicker validate[required]" data-live-search="true">
           ...
           <?php foreach($recetas as $receta){ ?>
              <option value="<?php echo $receta->id_receta; ?>"  <?php if($desayunos->receta == $receta->id_receta) echo "selected"; ?>><?php echo $receta->nombre; ?></option>
           <?php } ?>
           <?php } ?>
        </select>
      </div>
