<?php
class Modelo_Tiporecetas extends CI_Model {
	private $tabla;
	private $prefijo;
	
	function __construct(){
		$this->tabla = "tipos_de_receta";
		//$this->prefijo = substr($this->tabla, 0, 2) . "_";
		parent::__construct();
	}

	public function getLastId(){
		$this->db->select_max("id_tipo_receta","maximo");
    ...
    public function eliminar($codigo){
    	$this->db->where('id_tipo_receta', $codigo);
		$this->db->delete($this->tabla);
    }

    public function obtenerTiposRecetas($nombre_tipoReceta){
    	$this->db->select('*');
    	$this->db->from($this->tabla);
    	$this->db->like('nombre',$nombre_tipoReceta);
    	$result = $this->db->get();
