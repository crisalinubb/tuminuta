<?php
class Modelo_Regimen extends CI_Model {
	private $tabla;
	private $prefijo;
	
	function __construct(){
		$this->tabla = "regimenes";
		//$this->prefijo = substr($this->tabla, 0, 2) . "_";
		parent::__construct();
	}

	public function getLastId(){
		$this->db->select_max("id_regimen","maximo");
    ...
    public function eliminar($codigo){
    	$this->db->where('id_regimen', $codigo);
		$this->db->delete($this->tabla);
    }

    public function obtenerRegimen($nombre_regimen){
    	$this->db->select('*');
    	$this->db->from($this->tabla);
    	$this->db->like('nombre',$nombre_regimen);
    	$result = $this->db->get();
