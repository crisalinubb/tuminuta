<?php
class Modelo_Camas extends CI_Model {
	private $tabla;
	private $prefijo;
	...
	public function getLastId(){
		$this->db->select_max("id_cama","maximo");
    ...
    public function eliminar($codigo){
    	$this->db->where('id_cama', $codigo);
		$this->db->delete($this->tabla);
    }

    public function buscar_cama_por_sala($id_sala){
    	$this->db->select('*');
    	$this->db->from('camas');
    ...
    public function obtenerCamas($nombre_cama){
    	$this->db->select('*');
    	$this->db->from($this->tabla);
    	$this->db->like('cama',$nombre_cama);
    	$result = $this->db->get();

		return $result->result();
    }

    public function obtenerCama($servicio, $sala, $cama){
    	$this->db->select('*');
    	$this->db->from($this->tabla);
    	$this->db->where('codigo_servicio',$servicio);
    	$this->db->where('codigo_sala',$sala);
    	$this->db->where('id_cama',$cama);
    	$result = $this->db->get();
    	//die($this->db->last_query());
		return $result->result();
    }

    public function obtener_cama_servicio($servicio){
    	$this->db->select('*');
    	$this->db->from($this->tabla);
    	$this->db->where('codigo_servicio',$servicio);
    	$result = $this->db->get();
    ...
    public function obtener_cama_servicio_sala($servicio,$sala){
    	$this->db->select('*');
    	$this->db->from($this->tabla);
    	$this->db->where('codigo_servicio',$servicio);
    	$this->db->where('codigo_sala',$sala);
