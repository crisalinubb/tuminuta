<?php
class Modelo_aportesregimen extends CI_Model {
	private $tabla;
	private $prefijo;
	
	function __construct(){
		$this->tabla = "aportesnutricionales";
		//$this->prefijo = substr($this->tabla, 0, 2) . "_";
		parent::__construct();
	}

	public function getLastId(){
		$this->db->select_max("Id_Aporte_Nut","maximo");
    ...
    public function eliminar($codigo){
    	$this->db->where('Id_Aporte_Nut', $codigo);
		$this->db->delete($this->tabla);
    }

    public function obtener_Aporte_Regimen($id_regimen){
    	$this->db->select('*');
