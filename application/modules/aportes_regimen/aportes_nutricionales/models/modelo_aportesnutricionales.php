<?php
class Modelo_AportesNutricionales extends CI_Model {
	private $tabla;
	private $prefijo;
	
	function __construct(){
		$this->tabla = "aportes_nutricionales";
		//$this->prefijo = substr($this->tabla, 0, 2) . "_";
		parent::__construct();
	}

	public function getLastId(){
		$this->db->select_max("id_aporte_nutricional","maximo");
    ...
    public function eliminar($codigo){
    	$this->db->where('id_aporte_nutricional', $codigo);
		$this->db->delete($this->tabla);
    }

    public function obtenerAportesNutricionales($nombre_aporte){
    	$this->db->select('*');
    	$this->db->from($this->tabla);
    	$this->db->like('nombre',$nombre_aporte);
    	$result = $this->db->get();
