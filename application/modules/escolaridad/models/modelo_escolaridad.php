<?php
class Modelo_Escolaridad extends CI_Model {
	private $tabla;
	private $prefijo;
	
	function __construct(){
		$this->tabla = "escolaridad";
		//$this->prefijo = substr($this->tabla, 0, 2) . "_";
		parent::__construct();
	}

	public function getLastId(){
		$this->db->select_max("id_escolaridad","maximo");
    ...
    public function eliminar($codigo){
    	$this->db->where('id_escolaridad', $codigo);
		$this->db->delete($this->tabla);
    }

    public function obtenerEscolaridad($nombre_escolaridad){
    	$this->db->select('*');
    	$this->db->from($this->tabla);
    	$this->db->like('esc_nombre',$nombre_escolaridad);
    	$result = $this->db->get();
