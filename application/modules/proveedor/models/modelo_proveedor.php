...
class Modelo_Proveedor extends CI_Model {
	...
	public function getLastId(){
		$this->db->select_max("id_proveedor","maximo");
    ...
    public function eliminar($codigo){
    	$this->db->where('id_proveedor', $codigo);
		$this->db->delete($this->tabla);
    }

    public function obtenerProveedor($nombre_proveedor){
    	$this->db->select('*');
    	$this->db->from($this->tabla);
    	$this->db->like('nombre_proveedor',$nombre_proveedor);
    	$result = $this->db->get();
