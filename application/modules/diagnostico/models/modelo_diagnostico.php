<?php
class Modelo_Diagnostico extends CI_Model {
	private $tabla;
	private $prefijo;
	
	function __construct(){
		$this->tabla = "diagnostico";
		//$this->prefijo = substr($this->tabla, 0, 2) . "_";
		parent::__construct();
	}

	public function getLastId(){
		$this->db->select_max("id_diagnostico","maximo");
	...
	public function listar($where = false, $pagina = false, $cantidad = false){
		...
		$sql = $this->db->select('*')
				->from($this->tabla)
				->order_by('diagnostico_descripcion','asc')
				->get();
    ...
    public function eliminar($codigo){
    	$this->db->where('id_diagnostico', $codigo);
		$this->db->delete($this->tabla);
    }

    public function obtenerDiagnostico($nombre_diagnostico){
    	$this->db->select('*');
    	$this->db->from($this->tabla);
    	$this->db->like('diagnostico_descripcion',$nombre_diagnostico);
    	$result = $this->db->get();
